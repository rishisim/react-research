...
def parse_multi_trace_reflexion(file_path):
    try:
        with open(file_path, 'r') as f:
            data = json.load(f)
        rewards = []
        ...
        print(f"Warning: {file_path} not found. Skipping reflexion analysis.")
        return []
...
def main():
    ...
    fever_reflexion_rewards = parse_multi_trace_reflexion('FEVER_Experiment/react_multi_trace_reflexion_results.json')
    ...
    with open('analysis_results.csv', 'w', newline='') as csvfile:
        ...
        writer.writerow(['Dataset', 'Q', 'react_baseline', 'react_multi_trace', 'react_multi_trace_reflexion'])
        ...
        for i in range(len(fever_baseline_rewards)):
            reflexion_reward = fever_reflexion_rewards[i] if i < len(fever_reflexion_rewards) else ''
            writer.writerow(['FEVER', i + 1, fever_baseline_rewards[i], fever_multi_trace_rewards[i], reflexion_reward])
    ...
    if fever_reflexion_rewards:
        reflexion_correct_fever = sum(fever_reflexion_rewards)
        reflexion_accuracy_fever = (reflexion_correct_fever / len(fever_reflexion_rewards)) * 100
        print(f"  Multi-Trace with Reflexion Correct: {reflexion_correct_fever} / {len(fever_reflexion_rewards)}")
        print(f"  Multi-Trace with Reflexion Accuracy: {reflexion_accuracy_fever:.2f}%")
    ...
    if fever_reflexion_rewards:
        print("  Difference (Reflexion vs. Baseline):")
        print(f"    Correct Questions: {reflexion_correct_fever - baseline_correct_fever}")
        print(f"    Accuracy: {reflexion_accuracy_fever - baseline_accuracy_fever:.2f}%")
        print("  Difference (Reflexion vs. Multi-Trace):")
        print(f"    Correct Questions: {reflexion_correct_fever - multi_trace_correct_fever}")
        print(f"    Accuracy: {reflexion_accuracy_fever - multi_trace_accuracy_fever:.2f}%")
