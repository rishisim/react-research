"""
Reflexion Pruning Agent for HotpotQA
...
# Add agents/hotpotqa to path for local imports if needed
...
# Note: This assumes prog_ca_pruning is in the same directory or accessible
from prog_ca_pruning_agent import (
    run_prog_ca_pruning_hotpot,
...
def verify_answer(trajectory_info, verification_prompt, to_print=False):
    ...
    if isinstance(trajectory_info, dict):
        ...
        if not trajectory_str:
            ...
            trajectory_str = f"Question: {question}\n[Trajectory details not available]\nFinal Answer: {answer}"
    else:
...
def run_reflexion_pruning(idx, prompt_template=None, to_print=True, max_trials=7):
    """
    Run Reflexion with Programmatic CA Pruning for HotpotQA.
    ...
    if to_print:
        print("="*60)
        print(f"[FRAMEWORK] Reflexion Pruning (HotpotQA) (Up to {max_trials} Trials)")
        print("="*60)
...
{prompt_template}"""
        ...
        reward, trace_result = run_prog_ca_pruning_hotpot(
            idx=idx,
        ...
        # Check if the answer is correct using LLM-as-judge (standard for HotpotQA)
        llm_eval = llm_judge_answer(question_text, final_answer, gt_answer)
        is_correct = llm_eval['llm_correct']
        
        if to_print:
            print(f"[EVALUATION] LLM Judge: {'CORRECT' if is_correct else 'INCORRECT'} (GT: {gt_answer})")
    ...
    # Metrics
    # For HotpotQA, we usually trust the LLM judge for EM/Accuracy in this context, 
    # but the environment might return strict EM. trace_result['em'] is usually 0/1 from strict match.
    # We will use the final trace's strict EM for the 'em' field, but rely on judge for 'reward'.
    final_strict_em = traces[num_traces_run].get('em', 0.0)
    ...
    # Add verification calls (one per trial attempt)
    ...
    info_dict = {
        ...
        'gt_answer': gt_answer,
        'em': final_strict_em,
        'f1': traces[num_traces_run].get('f1', 0.0),
        'reward': 1.0 if is_correct else 0.0, # Use LLM judge for reward
        'n_calls': total_calls,
    ...
    if to_print:
        print("="*60)
        print(f"[FINAL] Answer: {final_answer} | GT: {gt_answer} | EM: {final_strict_em}")
    ...
    framework_folder = os.path.join(os.path.dirname(__file__), '../../../results/hotpotqa/reflexion_pruning')
...
if __name__ == '__main__':
    # Test with a sample HotpotQA example (idx 0)
    print("\n[TEST] Running Reflexion Pruning Agent test (max 7 trials)\n")
    reward, info = run_reflexion_pruning(idx=0, to_print=True, max_trials=7)
