"""
Test Action Prune ReAct Agent on 10 HotPotQA examples
"""
...
def main():
    ...
    # Generate 10 random indices from HotPotQA dev set (7405 examples)
    test_indices = random.sample(range(7405), num_examples)
    
    print("\n" + "="*70)
    print("HOTPOTQA ACTION PRUNE REACT - 10 EXAMPLES TEST")
    print("="*70)
    print(f"Seed: {seed}")
    ...
    for i, test_idx in enumerate(test_indices, 1):
        ...
        correct += info['em']
        total_f1 += info['f1']
        llm_correct += (1 if info.get('llm_correct', False) else 0)
        total_calls += info['n_calls']
        
        print(f"\n[Result {i}] Answer: {info['answer']} | GT: {info['gt_answer']} | EM: {info['em']} | F1: {info['f1']:.2f} | LLM: {info.get('llm_correct', False)}")
    
    # Calculate metrics
    em_accuracy = correct / num_examples
    avg_f1 = total_f1 / num_examples
    llm_accuracy = llm_correct / num_examples
    ...
    results_dir = (script_dir / "../../../results/hotpotqa/action_prune").resolve()
    ...
    print("="*70)
    print(f"EM Accuracy: {em_accuracy:.2%} ({correct}/{num_examples})")
    print(f"Average F1: {avg_f1:.2%}")
    print(f"LLM-Judge Accuracy: {llm_accuracy:.2%} ({llm_correct}/{num_examples})")
