"""
Combined Majority Voting and CoT-SC Agent for HotPotQA

Optimization: Runs 3 independent ReAct traces ONCE and applies both:
1. Majority Voting (Semantic)
...
def run_combined_agent(idx, prompt_template=None, to_print=True, num_traces=3):
    ...
    if to_print:
        print("="*60)
        print(f"[FRAMEWORK] COMBINED Optimized Run (Majority Vote + CoT-SC) - HotPotQA")
    ...
    # --- PHASE 2: Majority Voting Logic (Semantic) ---
    if to_print:
        print(f"\n{'='*60}")
        print("[LOGIC] Applying Semantic Majority Voting")
    
    mv_answer, mv_tokens = majority_vote_semantic(answers, question_text)
    mv_em = 1.0 if mv_answer == gt_answer else 0.0
    
    # Eval with detailed LLM Judge 
    mv_llm_eval = llm_judge_answer(question_text, mv_answer, gt_answer)

    # MV tokens include voting cost + all trace costs + judge cost
    mv_input_tokens = total_input_tokens + mv_tokens['input_tokens'] + mv_llm_eval.get('judge_input_tokens', 0)
    mv_output_tokens = total_output_tokens + mv_tokens['output_tokens'] + mv_llm_eval.get('judge_output_tokens', 0)
    ...
    mv_info_dict = {
        ...
        'input_tokens': mv_input_tokens,
        'output_tokens': mv_output_tokens,
        'total_tokens': mv_input_tokens + mv_output_tokens,
        'num_traces_run': num_traces,
        'individual_votes': answers,
        'trace_summaries': trace_summaries,
        'full_traces': all_traces,
        'llm_correct': mv_llm_eval['llm_correct'],
        'llm_explanation': mv_llm_eval['llm_explanation'],
        'framework': 'majority_voting'
    }
    
    if to_print:
        print(f"[MAJORITY] Answer: {mv_answer} | EM: {mv_em} | LLM: {mv_llm_eval['llm_correct']}")
    ...
    # Eval with detailed LLM Judge on CoT result
    cot_llm_eval = llm_judge_answer(question_text, cot_answer, gt_answer)

    # CoT-SC tokens include synthesis cost + all trace costs + judge cost
    cot_input_tokens = total_input_tokens + synthesis_tokens['input_tokens'] + cot_llm_eval.get('judge_input_tokens', 0)
    cot_output_tokens = total_output_tokens + synthesis_tokens['output_tokens'] + cot_llm_eval.get('judge_output_tokens', 0)
    ...
    cot_info_dict = {
        ...
        'trace_summaries': trace_summaries,
        'full_traces': all_traces,
        'llm_correct': cot_llm_eval['llm_correct'],
        'llm_explanation': cot_llm_eval['llm_explanation'],
        'framework': 'cot_sc'
    }
    
    if to_print:
        print(f"[COT-SC] Answer: {cot_answer} | EM: {cot_em} | LLM: {cot_llm_eval['llm_correct']}")
        print("="*60)
